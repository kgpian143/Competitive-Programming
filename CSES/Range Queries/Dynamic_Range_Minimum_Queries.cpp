...
void build( vector<int>&v ,  int ind , int st , int se   )
{
      if (st == se) {
        ...
        table[ind] = min(table[ind*2] , table[ind*2+1]);
    }
}
int minimum(  int v , int st, int se, int l, int r ) {
    if (l > r) 
        return INT_MAX;
    if (l == st && r == se) {
        return table[v];
    ...
    int tm = (st + se) / 2;
    return min(minimum(v*2, st, tm, l, min(r, tm))
           , minimum(v*2+1, tm+1, se, max(l, tm+1), r));
}
...
void update(int i, int st, int se , int pos, int new_val) {
    if (st == se ) {
        ...
        table[i] = min(table[i*2] , table[i*2+1]);
    }
}
int main()
{
